...
# Accept either a JSON string or a TOML table for the service account
svc_info = None
if "GOOGLE_SERVICE_ACCOUNT_JSON" in st.secrets:
    import json as _json
    svc_info = _json.loads(st.secrets["GOOGLE_SERVICE_ACCOUNT_JSON"])
...
def get_drive_service(_svc_info: dict):
    creds = Credentials.from_service_account_info(_svc_info, scopes=scopes)
    return build("drive", "v3", credentials=creds, cache_discovery=False)

svc = get_drive_service(svc_info)

def _list_epw_files_drive():
    """Return a list of dicts with id, name, size, modifiedTime for EPWs in the folder."""
...
# --------------- Load files ---------------
# Optional uploads
uploaded = st.file_uploader("Upload EPW files (optional)", type=["epw"], accept_multiple_files=True)
...
def _parse_selected(name: str):
    ...
    try:
        if item["source"] == "uploaded":
            ...
            tmp.write(data)
        tmp.flush(); tmp.close()
        df, location_dict, df_stats = parse_with_ladybug(tmp.name)
        df = df.copy()
...
with tab_compare:
    ...
    with st.expander("Selection",expanded=True):
        ...
        for name in multi_select:
            try:
                df, loc, stats = _parse_selected(name)
                label = label_from_location(loc, name.rsplit('.',1)[0])
